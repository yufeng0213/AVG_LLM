...
const initLoreEntryStates = () => {
  const states = {}
  for (const entry of WORLD_BOOK_ENTRY_DEFS) {
    states[entry.key] = false
  }
  // 默认展开第一个
  if (WORLD_BOOK_ENTRY_DEFS.length > 0) {
    states[WORLD_BOOK_ENTRY_DEFS[0].key] = true
  }
  loreEntryStates.value = states
}
initLoreEntryStates()

const toggleLoreEntry = (key) => {
  loreEntryStates.value[key] = !loreEntryStates.value[key]
}
