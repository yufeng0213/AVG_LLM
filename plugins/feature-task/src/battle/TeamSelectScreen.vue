    ...
    <div class="team-preview-section">
      ...
      <div class="team-slots">
        ...
        <div
          ...
          :key="char.id"
          class="team-slot team-card"
        >
          <template v-if="char.portraits?.[0]">
            <img :src="char.portraits[0]" :alt="char.name" class="slot-portrait" />
          </template>
          <template v-else>
            <span class="slot-avatar-fallback">&#x1F464;</span>
          </template>
          <div class="slot-name-bar">
            <span class="slot-name">{{ char.name }}</span>
            <span v-if="char.identity" class="slot-identity">{{ char.identity }}</span>
          </div>
        </div>
        <!-- 空位 -->
        <div
