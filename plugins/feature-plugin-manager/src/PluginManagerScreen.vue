...
const copyTextToClipboard = async (text) => {
  ...
  textarea.select()
  let ok = false
  try { ok = document.execCommand('copy') } catch { ok = false } finally { textarea.remove() }
  return ok
}
...
const handleCopyFeaturePluginOverrides = async () => {
  ...
  if (copyingFeatureOverrides.value) return
  try {
...
const applyFeaturePluginOverridesSource = (source, options = {}) => {
  ...
  if (applyingFeatureOverrides.value) return false
...
const triggerTextFileDownload = (filename, textContent) => {
  ...
  const blob = new Blob([String(textContent ?? '')], { type: 'application/json;charset=utf-8' })
...
const handleExportFeaturePluginOverridesFile = () => {
  if (exportingFeatureOverrides.value) return
  try {
...
const handleImportFeaturePluginOverridesFile = async () => {
  if (importingFeatureOverrides.value) return
  try {
    ...
    applyFeaturePluginOverridesSource(text, { successPrefix: `已导入 ${file.name}` })
  } catch (error) {
