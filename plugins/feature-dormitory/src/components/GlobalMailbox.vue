...
watch([() => props.isOpen, activeTab], ([isOpen, tab]) => {
  if (isOpen && tab === 'inbox') {
    startChecker('global', currentBookCharacters.value)
  }
})
</script>
...
.mailbox-coin-box {
  ...
  align-items: center;
  gap: 4px;
  background: color-mix(in srgb, var(--accent-yellow, #F5C542) 8%, transparent);
  border: 1px solid color-mix(in srgb, var(--accent-yellow, #F5C542) 15%, transparent);
  border-radius: 10px;
  padding: 4px 10px;
}
...
.letter-waiting p { margin: 0; font-size: 0.9rem; color: color-mix(in srgb, var(--accent-yellow, #F5C542) 50%, transparent); }
