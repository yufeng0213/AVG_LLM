...
// ====== Toast ======
function showToastFn(msg, type = 'success') {
  toastKeyRef.value++
  toastMsg.value = msg
...
function handleSignIn() {
  ...
  setTimeout(() => {
    ...
    const itemNames = reward.items.length > 0
      ...
      : ''
    const msg = `+${reward.baseCoins}💰${reward.label ? ' (' + reward.label + ')' : ''}${itemNames ? ' + ' + itemNames : ''}`
...
function handleClaimBonus() {
  ...
  if (result) {
    ...
    const itemNames = result.items.length > 0
      ...
      : ''
    const msg = `🎁 月度奖励: +${result.coins}💰${itemNames ? ' + ' + itemNames : ''}`
    showToastFn(msg, 'success')
  } else {
    showToastFn('暂无可领取的月度奖励', 'info')
  }
