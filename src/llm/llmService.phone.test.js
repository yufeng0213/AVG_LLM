...
export const generateGroupChatReply = async (params = {}) => {
  ...
  const recentChat = (contextMessages > 0 ? history.slice(-contextMessages) : [])
    .map(item => {
      if (item.role === 'user') return `玩家: ${String(item.text || '').trim()}`
      return `${item.senderName || '角色'}: ${String(item.text || '').trim()}`
    })
    .filter(Boolean)
    .join('\n')
  ...
  const userPrompt = [
    ...
    groupType === 'worldbook' ? `【世界书上下文】\n${worldContext}` : worldContext,
    `【可用发言角色（请根据人设和上下文决定谁说话）】\n${memberListText}`,
    recentChat ? `【最近群聊记录】\n${recentChat}` : '【最近群聊记录】\n（这是群聊的第一条消息）',
    userMessage ? `【玩家刚发送】${userMessage}` : '',
    '请根据角色人设和上下文，决定哪些角色（0-3个）主动发言。',
