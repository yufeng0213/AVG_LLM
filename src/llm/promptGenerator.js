...
const buildInstructionSection = (
  ...
  lines.push('')
  lines.push('字段说明：')
  lines.push('- event: 记录本轮重要事件')
  lines.push('  - t: conversation/romance/conflict/milestone/gift/other')
  lines.push('  - p: 参与者角色名（不用ID），多人用逗号分隔')
  lines.push('  - s: 20字内摘要')
  lines.push('- mem: 角色对玩家的印象变化')
  lines.push('  - c: 角色名')
  lines.push('  - txt: 印象描述（如好感加深、产生怀疑等）')
  lines.push('- loc: 发现的新地点')
  lines.push('  - n: 地点名称')
  lines.push('- impact: 玩家行为对世界/NPC的影响（可选）')
  lines.push('  - p: 玩家做了什么')
  lines.push('  - e: 产生的影响')
  lines.push('  - c: 受影响的角色（可多个，逗号分隔）')
  lines.push('- sms: 当前在同一地点的两个NPC可能发的短信（可选）')
  lines.push('  - a/b: 两个NPC的名字')
  lines.push('  - loc: 所在地点')
  lines.push('  - txt: 短信话题（如"聊聊刚才的事"）')
  lines.push('')
  lines.push('输出建议：')
  lines.push('- 每轮建议输出aux，记录事件和印象')
  lines.push('- impact在玩家做出重要选择时输出')
  lines.push('- sms在两个NPC同地点且有互动可能时输出')
