...
const buildInstructionSection = (
  ...
  lines.push('')
  lines.push('### 思考步骤（在 <thinking> 中逐步分析）')
  lines.push('注意：<thinking> 必须用编剧内心独白方式思考，禁止出现"我应该"、"我需要生成"、"用户要求"、"作为AI"等元话术。')
  lines.push('')
  lines.push('**剧情构思**：')
  ...
  lines.push('')
  lines.push('**aux 内容分析（必须逐条思考）**：')
  lines.push('6. 这轮有哪些有意义的交互？（闲聊/无实质内容的跳过，有意义的才记录）')
  lines.push('7. 具体发生了什么事件？事件类型是什么？参与者是谁？用一句话概括')
  lines.push('8. 哪个角色对玩家（或其他角色）产生了主观印象变化？具体是什么印象？')
  lines.push('9. 是否有首次出现的新地点？')
  lines.push('10. 玩家的行为是否对世界或NPC产生了某种影响？')
  lines.push('11. 当前地点是否有两个NPC可能私下交流（发短信）？话题是什么？')
  lines.push('')
  lines.push('**自检清单**：')
  lines.push('- 这轮推动了什么价值变化？（好感/信任/局势/角色成长，至少一项）')
  lines.push('- 内心独白够不够？（角色需要有内在反应，不只外部动作）')
  lines.push('- 信息是螺旋式释放的吗？（别一次性全抖出来，保留悬念）')
  lines.push('- 有没有意外感？（日常场景也要有小转折）')
  lines.push('- aux是否已分析完毕？event必须至少一条')
  lines.push('')
  ...
  lines.push('### 示例')
  lines.push('<thinking>')
  lines.push('当前是傍晚，两人在回家路上。林夏表现出担忧...')
  lines.push('接下来推进到夜晚，场景切换到家中客厅。')
  lines.push('')
  lines.push('【aux分析】')
  lines.push('1. 有意义的交互：林夏主动关心玩家，玩家回应，这是情感交流')
  lines.push('2. 事件：林夏与玩家深夜谈心 → event: t="conversation", p="林夏,玩家", s="林夏关心玩家心事"')
  lines.push('3. 印象变化：林夏觉得玩家有些疲惫，想了解原因 → mem: c="林夏", txt="想了解玩家心事，产生关心"')
  lines.push('4. 新地点：首次进入家中客厅 → loc: n="家中客厅"')
  lines.push('5. 无明显影响，无NPC短信')
  lines.push('</thinking>')
  ...
  lines.push('</choices>')
  lines.push('<aux>')
  lines.push('  <event t="conversation" p="林夏,玩家" s="林夏关心玩家心事"/>')
  lines.push('  <mem c="林夏" txt="想了解玩家心事，产生关心"/>')
  lines.push('  <loc n="家中客厅"/>')
  lines.push('</aux>')
  
  // 如果世界书有场景配置，列出可用场景（只显示名称，不显示id）
  if (worldBook?.scenes && worldBook.scenes.length > 0) {
    lines.push('')
    lines.push('### 可用场景列表')
    const sceneList = worldBook.scenes.slice(0, 20)
    for (const scene of sceneList) {
      lines.push(`- ${scene.name}${scene.description ? ` (${scene.description})` : ''}`)
    }
  ...
  // 辅助输出区块（强制输出，用于记录剧情事件和角色印象）
  lines.push('')
  lines.push('### 剧情记录区块（⚠️ 强制输出）')
  lines.push('剧情生成完成后，在 <choices> 之后**必须**输出 <aux> 区块：')
  lines.push('')
  lines.push('<aux>')
  lines.push('  <event t="类型" p="角色名" s="事件摘要"/>')
  lines.push('  <mem c="角色名" txt="对玩家的印象变化"/>')
  lines.push('  <loc n="新地点名"/>')
  lines.push('  <impact p="玩家行为" e="影响描述" c="受影响角色"/>')
  lines.push('  <sms a="NPC名" b="NPC名" loc="地点" txt="话题摘要"/>')
  lines.push('</aux>')
  lines.push('')
  lines.push('⚠️ **强制规则**：')
  lines.push('- 每轮必须输出 <aux> 区块')
  lines.push('- 每轮必须至少输出一条 <event> 或 <mem>')
  lines.push('- 即使本轮没有明显事件，也要记录一条"日常互动"类型的event')
  lines.push('- 如果角色对玩家有任何态度变化，必须输出对应的 <mem>')
  lines.push('')
  lines.push('### aux各字段含义详解')
  lines.push('')
  lines.push('**event（事件记录）**：记录这轮剧情中发生了什么事')
  lines.push('- 用途：让系统记住"这轮发生了什么重要事件"，用于后续剧情回顾和事件追踪')
  lines.push('- t: 事件类型（conversation=对话交流, romance=浪漫进展, conflict=冲突, milestone=里程碑, gift=送礼, daily=日常, other=其他）')
  lines.push('- p: 参与者角色名（多人用逗号分隔）')
  lines.push('- s: 事件摘要（20字内，如"两人深夜谈心"）')
  lines.push('')
  lines.push('**mem（角色印象）**：记录这轮中某个角色对玩家的看法/印象有什么变化')
  lines.push('- 用途：让角色"记住"玩家的行为，影响后续该角色对玩家的态度和对话风格')
  lines.push('- c: 角色名')
  lines.push('- txt: 印象变化描述（如"觉得玩家很温柔"、"对玩家产生怀疑"、"好感明显上升"）')
  lines.push('')
  lines.push('**loc（新地点）**：记录这轮中玩家首次发现/进入的新地点')
  lines.push('- 用途：解锁新地点，让系统知道玩家去过哪里')
  lines.push('- n: 地点名称')
  lines.push('')
  lines.push('**impact（行为影响）**：记录玩家的某个行为对世界或其他NPC产生了什么影响')
  lines.push('- 用途：追踪玩家行为对世界状态的改变，如帮助某人后那人处境改善')
  lines.push('- p: 玩家做了什么行为')
  lines.push('- e: 这个行为产生的影响')
  lines.push('- c: 受影响的角色（可多个，逗号分隔）')
  lines.push('')
  lines.push('**sms（NPC私下短信）**：记录当前同一地点的两个NPC可能私下发的短信')
  lines.push('- 用途：让NPC之间有私下交流，增加世界真实感')
  lines.push('- a/b: 两个NPC的名字')
  lines.push('- loc: 他们所在的地点')
  lines.push('- txt: 短信话题摘要（如"聊聊刚才玩家做的事"）')
